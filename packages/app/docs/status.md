...
These values are in the code today and will change in a final "freeze" before launch. They are shown throughout this documentation as they stand, read from the code at build time, so they are never out of date — but they are not yet final.
...
| **The reserved-name list** | A short battery-grade list | A curated list of about a thousand names, reviewed and applied before the freeze — the one input that cannot be corrected afterwards, because a name left off is anyone's the block after launch ([Names](names)) |
