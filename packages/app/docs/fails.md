...
The value sits at the address that received it, which owes you a refund. The settlement service pays it in a separate transaction; the debt is visible in the log until it does.
...
One exception crosses the line: an amount below {{nim:REFUND_FLOOR}} that would otherwise be refunded is kept (`BELOW_REFUND_FLOOR`). Without that floor, thousands of trivially underfunded messages would become thousands of refund transactions someone has to send.
...
The wallet returning a transaction hash means the transaction was accepted for broadcast. It does not mean it will land. Three things make Nimiq accept a transaction and then drop it without any error:
