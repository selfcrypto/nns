...
- **Transfer Ownership.** Type the new owner's address or name. The transfer completes after **{{dur:XFER_TIMELOCK}}**. Until then you keep full control and **Cancel Transfer** undoes it, with the time left on the tile. To change the recipient, cancel and transfer again. Check the recipient in the wallet's sheet: the delay guards a mistyped address, not a stolen key. On completion the name points at the new owner, and the linked EVM address, the host and any open sale are cleared.
...
- **Sell (Fixed Price).** Puts the name up for sale at a price you set, no lower than the base registration price (today {{nim:FEE_BASE}}). Listing is free. You can take it off sale at any time, and it expires by itself after {{dur:OFFER_MAX_LIFETIME}}. You receive the price minus {{pct:COMMISSION_RATE}} commission.
