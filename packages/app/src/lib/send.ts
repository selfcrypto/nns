...
export async function performSend(options: {
  ...
  for (let round = 0; round < rounds; round += 1) {
    try {
      if (await confirm.poll(submitted.hash)) return { status: 'confirmed' }
    } catch {
      // A flaky poll is not a failed send; the next round asks again.
    }
    await sleep(intervalMs)
  }
  // Honest wording downstream: not "failed" — the network did not show the
  // effect within the window, which is all anyone can truthfully say.
