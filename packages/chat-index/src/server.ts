/**
 * The read endpoint. Two routes, CORS open to any origin — the app is a
  ...
  return createServer((request, response) => {
    void (async () => {
      ...
      if (match === null) {
        send(response, 404, {
          error: 'UNKNOWN_ROUTE',
          message: 'this service serves /messages/{address} and /healthz',
        })
        return
      }
      ...
      try {
        ...
        send(response, 200, {
          ...
          messages: rows.map((row) => ({
            ...
            // Only executed transactions are ever stored, so this is constant —
            // it is here because the client's parser takes the chain's shape.
