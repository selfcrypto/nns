/**
 * Structured logging, one JSON object per line. Its own copy, as
 * `chat-index` keeps one: this service imports `core` for the rules and
...
export function createLogger(level: LogLevel = 'info', write: (line: string) => void = (line) => process.stdout.write(line)): Logger {
  const emit = (severity: LogLevel, event: string, fields?: LogFields): void => {
    ...
    const payload: LogFields = { level: severity, component: 'notify', event, ...fields }
