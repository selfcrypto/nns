...
|---|---|
| `@nns/core` | Builds and parses messages; verifies §8.1/§8.2 bytes. Pure |
| `@nns/anchor` | Reads §9 anchors; ships the contract and its compiled artifact |
| `@nns/resolver` | Resolves a name with quorum and proof verification. **The one most people install** |

`@nns/resolver` also ships `dist/nns.js` — the same source bundled into one
...
The `@nns` scope must exist on npmjs.com and the publishing account must own
it. Create the organisation in the web UI — an npm account cannot claim a
scope by publishing into it — then:

```sh
npm login                       # the account that owns @nns
...
```sh
pnpm -r --filter "@nns/core" --filter "@nns/anchor" --filter "@nns/resolver" \
...
```sh
pnpm --filter @nns/resolver exec npm pack --dry-run
```

`@nns/resolver` must list `dist/nns.js`, `dist/rendering.css` and
`dist/index.d.ts`. `@nns/core` must list `vectors/`.
...
```sh
pnpm publish --filter @nns/core --access public
pnpm publish --filter @nns/anchor --access public
pnpm publish --filter @nns/resolver --access public
```
...
```sh
cd "$(mktemp -d)" && npm init -y >/dev/null && npm install @nns/resolver
...
```sh
curl -sI https://cdn.jsdelivr.net/npm/@nns/resolver/dist/nns.js | head -3
```
...
- **`@nns/anchor` keeps `viem` and `solc` as devDependencies**, because no
  ...
  install of `@nns/resolver`, for code a consumer cannot call.
