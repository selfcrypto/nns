...
|---|---|---|
| **Pending transfer** | `pending.transfer` | "Transferring to `<newOwner>` ≈ date" (matures at `effectiveHeight` = send height + `XFER_TIMELOCK`, ~12 h). Until maturity the name resolves as before and the current owner retains full control. Cancellable with `K`. A second `X` supersedes and restarts the clock, and since r30 an `O` voids it. **Never beside an offer** (§6 `X`, r30), the exclusivity an auction already had |
| **Open offer** | `pending.offer` | "For sale at `<price>` NIM." **Never beside a pending transfer** (§6 `X`, r30). Irrevocable until `openedHeight + OFFER_IRREVOCABLE` (~2.4 h) — show that plainly to the seller *before* §4's `O` is sent and while it holds; cancellable with `K` after; auto-expires at `openedHeight + OFFER_MAX_LIFETIME` (~15 d) |
...
Since r30 an `X` maturing can find neither an offer nor another transfer —
the two are exclusive and a second `X` supersedes rather than joins — so on
that path the resets that still do work are the host, the target and the EVM
record; the offer reset belongs to the `B` and auction-close paths.
...
| `X` transfer | Owner | REGISTERED, no open auction, **no open offer** (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), name listed (`OFFER_OPEN`) | Matures at height + `XFER_TIMELOCK` (~12 h). A second `X` supersedes the first and restarts the clock. The timelock guards a mistyped recipient, not a thief — never describe it as security against a stolen key. **A listed name refuses the transfer** (§6 `X`, r30): the way out is a `K`, so the disabled tile says *take it off sale* — except inside `OFFER_IRREVOCABLE`, where a `K` cannot lift the offer either and the tile carries the wait in `Gate.blocksLeft` instead, exactly as the cancel tile does. Advice the protocol refuses to take is worse than no advice |
...
| `O` offer | Owner | REGISTERED, no open offer, no open auction — **a pending `X` does not block it** (r30: listing voids the transfer, and the review says so) | GRACE, absent, auction open (`AUCTION_OPEN`) | Price ≥ `minPrice` (`FEE_BASE` at the current height, from `/params` — never a constant). Carries `DUST_VALUE` (listing fee is 0). Disclose the irrevocable window **before** sending |
