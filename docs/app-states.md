...
| `O` offer | Owner | REGISTERED, no open offer, no open auction | GRACE, absent, auction open (`AUCTION_OPEN`) | Price ≥ `minPrice` (`FEE_BASE` at the current height, from `/params` — never a constant). Carries `DUST_VALUE` (listing fee is 0). Disclose the irrevocable window **before** sending |
