...
| `A` auction (r28) | Owner | REGISTERED, no open auction — a pending `X` or open `O` does not block it (opening voids both; the review says so) | GRACE, absent, auction open (`AUCTION_OPEN`) | Reserve ≥ `minPrice` — `encodeAuction` refuses less, and the reason (the increment rounds to zero at a token reserve) is core's, not the app's. Duration in days, floor `AUCTION_MIN_DURATION` (~1 d), with `AUCTION_LANDING_MARGIN` (~1 h) added on top because §6 `A` measures from the landing block. Carries `DUST_VALUE`. Says that neither the auction nor a bid can be withdrawn, and **warns when the end is at or past expiry** (renew first) |
