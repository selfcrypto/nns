...
|---|---|---|
| **Pending transfer** | `pending.transfer` | "Transferring to `<newOwner>` ≈ date" (matures at `effectiveHeight` = send height + `XFER_TIMELOCK`, ~12 h). Until maturity the name resolves as before and the current owner retains full control. Cancellable with `K` at any time. A second `X` replaces it and restarts the clock (the way to change the recipient); an `O` or an `A` is refused while it stands (`TRANSFER_PENDING`, r30). **Never beside an offer or an auction** — one pending thing per name (§7.3) |
| **Open offer** | `pending.offer` | "For sale at `<price>` NIM." **Never beside a pending transfer or an auction** — one pending thing per name (§7.3, r30). Cancellable with `K` at any time (no irrevocable window since r30); a second `O` replaces it (the way to change the price); an `X` or an `A` is refused while it stands (`OFFER_OPEN`); auto-expires at `openedHeight + OFFER_MAX_LIFETIME` (~15 d) |
| **Open auction** (r28) | `pending.auction` | "Up for auction — starting price `<starting_price>` NIM, ends ≈ date." followed by the standing bid ("Highest bid `<bid>` NIM from `<bidder>`") or "No bids yet.", and "Next bid: at least `<minimumBid>` NIM." — the API's figure, `core.requiredBid`, never computed in the app. **Never beside an offer or a pending transfer** (§7.3, one pending thing per name). Not cancellable, does not expire by itself: it ends at `endHeight`, which a late bid moves out by `AUCTION_EXTENSION` (~10 min). An end at or past expiry is not refused — the grace reset cancels it and refunds the bid — so the Auction sheet warns *before* the `A` goes out |
...
Since r30 a name holds one pending thing, so an `X` maturing can find neither
an offer nor an auction; on that path the resets that still do work are the
host, the target and the EVM record, and the offer reset belongs to the `B`
and auction-close paths. On entering GRACE: host cleared, offers, pending `X`
and a running auction cancelled (the standing bid refunded). Nothing of one kind
voids another: an `X`, `O` or `A` over a pending thing of another kind is
refused, and the same kind replaces it.
The detail screen must not keep showing state the reset has already
destroyed.
...
|---|---|---|---|---|
| `G` register | Anyone | Name AVAILABLE (incl. released-by-`U`) | REGISTERED (`LOST_REGISTRATION_RACE` — a refund, but the UI never offers it), GRACE (`NAME_IN_GRACE`), reserved (`RESERVED_NAME`) | Value = the band fee from `/params.fees` **exactly** (§10.5) — the yearly figure, or the lifetime one when the sheet's term choice is Lifetime (§10.4: a hundred terms for ten fees, reviewed as the date it reaches, 2026-09-11); a surplus ≥ `REFUND_FLOOR` is refunded by the treasury on an `OK` line (2026-09-10), a shortfall in full. A same-block race is possible even after an availability check (`LOST_REGISTRATION_RACE` refunds) |
...
| `X` transfer | Owner | REGISTERED, nothing of another kind pending (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), name listed (`OFFER_OPEN`) | Matures at height + `XFER_TIMELOCK` (~12 h). A second `X` replaces the first and restarts the clock: on a name with a transfer pending the tile reads **Change Recipient** and the sheet prefills the current one. The timelock guards a mistyped recipient, not a thief — never describe it as security against a stolen key. **The tile closes on whatever is pending** and the line names it and the way out: *cancel that first* (one pending thing per name, §7.3) |
...
| `K` cancel | Owner | Something cancellable pending: a transfer or a sale, at any height | Nothing pending (`NOTHING_TO_CANCEL`) — incl. all of GRACE, whose entry already cancelled everything, and an open auction, which `K` cannot touch (bids are commitments; say *that*, not "nothing pending") | One `K` clears the name's one pending thing; the tile is named after it (`states.cancellable` is the one reading, shared by the tile, the sheet's label, its review and the confirm poll). No irrevocable window since r30: a sale listed a block ago cancels |
| `N` renew | **Anyone** | REGISTERED or GRACE | AVAILABLE (`NAME_NOT_FOUND` — the record is gone) | Extends from current expiry, not from now — early renewal is never penalised; say so. Value = band fee exactly, yearly or lifetime by the same choice, and the review shows the new expiry as ≈ date (2026-09-11). On Buy it is *Gift a renewal* for a name the viewer does not hold, and the review says the name stays the owner's (2026-09-09) |
| `O` offer | Owner | REGISTERED, nothing of another kind pending (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), transfer pending (`TRANSFER_PENDING`) | A second `O` replaces the listing: on a listed name the tile reads **Change Price** and the sheet prefills the current one. Price ≥ `minPrice` (`FEE_BASE` at the current height, from `/params` — never a constant). Carries `DUST_VALUE` (listing fee is 0). Cancellable at any time; say so, and the auto-expiry |
...
| `A` auction (r28) | Owner | REGISTERED, nothing pending (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), name listed (`OFFER_OPEN`), transfer pending (`TRANSFER_PENDING`) | Starting price ≥ `minPrice` — `encodeAuction` refuses less, and the reason (the increment rounds to zero at a token starting price) is core's, not the app's. Duration in days, floor `AUCTION_MIN_DURATION` (~1 d) and cap `AUCTION_MAX_DURATION` (~7 d, r31 fold), with `AUCTION_LANDING_MARGIN` (~1 h) added on top because §6 `A` measures from the landing block — clamped to the cap, which needs no margin. Carries `DUST_VALUE`. Says that neither the auction nor a bid can be withdrawn, and **refuses an end at or past expiry** (`AUCTION_BEYOND_TERM` since 2026-09-03 — renew first) |
...
| **A link filled the fields** | `?amount=` goes into the amount field as written and `?message=` into the reference field, **locked** with an Edit beside it and "Came with the payment link." under it; `?asset=usdt` opens the screen in USDT mode (app-ux §3) |
| Reference over 64 bytes | "Too long — a message travels in 64 bytes." The counter reads bytes, not characters, and the Pay button is disabled: 65+ is accepted by the RPC, returns a hash, and never lands in a block (§5.1) |
| Reference starting `NNS1` | "A message can't start with NNS1 — that is how a name message is written." Every indexer would read it as a protocol message (§7.5). Exact case: a lowercase `nns1` is invisible to every reader and is not refused |
| A reference in USDT mode | "A USDT payment carries no message. Pay in NIM to send one." Said, never dropped in silence — an ERC-20 `transfer` has nowhere to put one |
