...
| `A` auction (r28) | Owner | REGISTERED, no open auction — a pending `X` or open `O` does not block it (opening voids both; the review says so) | GRACE, absent, auction open (`AUCTION_OPEN`) | Reserve ≥ `minPrice` — `encodeAuction` refuses less, and the reason (the increment rounds to zero at a token reserve) is core's, not the app's. Duration in days, floor `AUCTION_MIN_DURATION` (~1 d), with `AUCTION_LANDING_MARGIN` (~1 h) added on top because §6 `A` measures from the landing block. Carries `DUST_VALUE`. Says that neither the auction nor a bid can be withdrawn, and **refuses an end at or past expiry** (`AUCTION_BEYOND_TERM` since 2026-09-03 — renew first) |
...
| **Marketplace** | `/offers` and `/auctions` in one list; each offer's name, price, seller (with identicon); each auction's name, an "Auction" badge, the standing bid or the reserve, the minimum next bid, the seller and the end as ≈ date; the custodial nature disclosed on the buy and bid paths (§4 `B`) |
