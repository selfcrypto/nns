...
and a running auction cancelled (the standing bid refunded). Nothing voids
anything on opening: an `X`, `O` or `A` over whatever is pending is refused.
...
| `X` transfer | Owner | REGISTERED, nothing pending (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), name listed (`OFFER_OPEN`), transfer pending (`TRANSFER_PENDING`) | Matures at height + `XFER_TIMELOCK` (~12 h). Nothing supersedes it: to change the recipient, cancel and send again. The timelock guards a mistyped recipient, not a thief — never describe it as security against a stolen key. **The tile closes on whatever is pending** and the line names it and the way out: *cancel that first* (one pending thing per name, §7.3) |
...
| `O` offer | Owner | REGISTERED, nothing pending (r30) | GRACE, absent, auction open (`AUCTION_OPEN`), name listed (`OFFER_OPEN` — a reprice is a cancel and a fresh listing), transfer pending (`TRANSFER_PENDING`) | Price ≥ `minPrice` (`FEE_BASE` at the current height, from `/params` — never a constant). Carries `DUST_VALUE` (listing fee is 0). Cancellable at any time; say so, and the auto-expiry |
