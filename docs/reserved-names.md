...
4. It is a consensus input: **rebuild both boxes** (`nns-vps rebuild <role>`,
   `docs/runbooks/deploy.md`), which on the demo forfeits `nimiqnames` and
   `satoshi` as above.
5. Every pass before `tasks/08` step 7 is the last cheap one. After
